...
export default function BanglaSection() {
  const [vowels, setVowels] = useState(STATIC_VOWELS)
  const [consonants, setConsonants] = useState(STATIC_CONSONANTS)

  useEffect(() => {
    Promise.all([api.bangla.getVowels(), api.bangla.getConsonants()])
      .then(([v, c]) => { if (v.length) setVowels(v); if (c.length) setConsonants(c) })
      .catch(() => {})
  }, [])
